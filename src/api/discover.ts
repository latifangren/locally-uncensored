...
export function getMainstreamTextModels(): DiscoverModel[] {
  return sortByRelease([
    ...
    { name: 'Gemma 4 12B (Q8 Full Quality)', description: 'Google Gemma 4 12B — Q8_0, near-lossless full quality. Native tools + vision, 128K context, ~13 GB. lmstudio-community build with a FIXED tool-template, so tool calling works in LM Studio out of the box. Fits a 12 GB GPU with minor offload.', pulls: '100K+', tags: ['12B', 'Q8_0', '13 GB'], updated: 'Hot', agent: true, released: '2026-04', downloadUrl: HF('lmstudio-community/gemma-4-12b-it-GGUF', 'gemma-4-12B-it-Q8_0.gguf'), filename: 'gemma-4-12B-it-Q8_0.gguf', sizeGB: 13 },
